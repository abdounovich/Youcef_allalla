...
$bot->reply(ButtonTemplate::create('للإستمرار إضغط على زر من القائمة أسفله 👇')
...
$botman->hears('steps', function($bot) {
    ...
    $bot->reply('   لتسهيل عملية الشراء إختصرناها  لك في 3 مراحل بسيطة  للغاية  😁 : ');
    $bot->typesAndWaits(1);
    ...
    $bot->reply(' 3⃣ :   أدخل العنوان الذي تـريد أن نـرسل إليه طلـبـيـتك    ');
    ...
    $bot->reply(' ⭕ ملاحظـــــة هــــــــامة ⭕ ');
    ...
    $bot->reply(' لزبون الحق في ارجاع أو تغيير المنتوج مع توضيح السبب وذلك خلال 24 ساعة الاولى بعد تأكيد استلام المنتوج من طرف نظام YALIDINE، وذلك بعد إعادة تغليف المنتوج كما كان وإرساله مع خدمة البريد السريع YALIDINE');
...
$botman->hears('product_([0-9]+)', function($bot,$number1) {
    ...
    foreach ($products as $product ) {
        ...
        ->image($product->photo)
        ->addButton(ElementButton::create(' 🛒 إشتر هذا الموديل ')
            ->payload($payload)
          ->type('postback'))
...
if ($commandes1->count()=="0" AND $commandes2->count()=="0" AND $commandes3->count()=="0" AND $commandes6->count()=="0") {
    $bot->reply("  لا توجد أي طلبية مسجلة بإسمك  😓  ");
    $bot->reply(ButtonTemplate::create('  إظغط على أحد الأزرار من القائمة أسفله   👇 ')
->addButton(ElementButton::create('  🛒 إبدأ التسوق الآن    ')
 ->type('postback')
 ->payload('show_me_products')
)
